...
double maxSingleForce = 1000000;

//the threshold for minimal strength
double minSingleForce = -50000;
...
double Sensors::getLength(double force)
{
	return force2length(force);
}

double Sensors::force2length(double force)
{
...
double Sensors::getAngle()
{
  ...
  return double(valid_angle)/valid_limit/100;
  //Serial.print("angle:");
...
{
	hx1.setPin(SCK1, DOUT1);
	hx2.setPin(SCK2, DOUT2);
	hx1.set_offset(bias1);
	hx2.set_offset(bias2);
}

void Sensors::initnRF24L01()
{
  //---------------------------------------------------------------wireless
...
double Sensors::getAngleRadius(double angle)
{
